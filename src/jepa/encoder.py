...
class SpatioTemporalViTEncoder(nn.Module):
	...
	def _spatio_temporal_positional_encoding(self) -> torch.Tensor:
		row = self.row_pos[:, :, None, :]
		col = self.col_pos[:, None, :, :]
		spatial = (row + col).reshape(1, self.patches_per_frame, self.embed_dim)

		temporal = self.time_pos[:, :, None, :]
		spatial = spatial[:, None, :, :]

		pos = temporal + spatial
		return pos.reshape(1, self.num_frames * self.patches_per_frame, self.embed_dim)
	...
	def _to_b_t_c_h_w(self, frames: torch.Tensor) -> torch.Tensor:
		...
		if b_t_c_h_w.shape[1] != self.num_frames:
			raise ValueError(f"Expected {self.num_frames} stacked frames, got {b_t_c_h_w.shape[1]}")
	...
	def forward(self, frames: torch.Tensor) -> torch.Tensor:
		...
		patch_tokens = patch_tokens.reshape(batch_size, num_frames * self.patches_per_frame, self.embed_dim)
